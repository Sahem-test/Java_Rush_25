package main.java.core.level08.task134;

/**
 * Личный калькулятор для быстрого подсчета
 * Вообразите, что вы создаете маленький карманный калькулятор для своего друга, который постоянно забывает, как складывать числа,
 * особенно когда это нужно сделать очень быстро. Вам нужен специальный инструмент для этой простой, но такой необходимой операции.
 * Создайте класс под названием MathMagic – это будет ваш персональный интеллектуальный центр для математических задач.
 * Внутри этого класса реализуйте статический метод showSum, который будет принимать два целых числа, например, первоеЧисло и второеЧисло.
