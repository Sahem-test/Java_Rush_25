package main.java.core.level08.task140;

/**
 * Проектирование различных форм и размеров объектов 📐
 * Вообразите, что вы помогаете инженеру-дизайнеру, которому нужно быстро создавать и проверять параметры различных прямоугольных объектов — от стен зданий до игровых элементов.
 * Ваша задача — создать инструмент, который мгновенно покажет размеры любого заданного прямоугольника, чтобы можно было легко экспериментировать с формами. Сконструируйте класс ShapeDesigner.
 * Внутри него объявите статический метод printRectangleSpecs, который будет принимать два целых числа: rectangleWidth и rectangleHeight.
 * Этот метод должен выводить на экран четкую информацию: "Характеристики прямоугольника: ширина = " с указанием ширины, а затем ", высота = " с указанием высоты.
