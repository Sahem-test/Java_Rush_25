package main.java.core.level09.task160;

import java.util.Locale;

/**
 * Форматирование отчёта с процентом и локалью
 * Вы – архитектор образовательной платформы, которая работает по всему миру.
 * После завершения теста студенту необходимо показать его результат в процентах.
 * Для европейских студентов особенно важно, чтобы десятичный разделитель был запятой, а не точкой.
 * Это вопрос не только правильности, но и уважения к культурным особенностям.
 ...
 * Затем, вычислив процент правильных ответов, используя String.format и локаль Locale.GERMANY, выведите на экран строку,
 * которая покажет имя студента и его процентный результат с одним знаком после запятой,
 * где десятичным разделителем будет запятая, а не точка, и знак процента будет явно указан. Например: "Анна: 90,0% правильных ответов".
 * Требования:
...
public class Task160 {
    public static void main(String[] args) {
        ...
        double percentRightAnswer =(double) correctAnswers/totalQuestions * 100;
        String result = String.format(Locale.GERMANY, "%s: %.1f%% правильных ответов", studentName, percentRightAnswer);
